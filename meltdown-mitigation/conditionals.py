...
COLOURS = {80: 'green', 60: 'orange', 30: 'red'}
STATUS = {90: 'LOW', 110: 'NORMAL'}
...
def reactor_efficiency(voltage: float, current: float, theoretical_max_power: float) -> str:
    ...
    for threshold, colors in COLOURS.items():
...
def fail_safe(temperature: float, neutrons_produced_per_second: float, threshold: float) -> str:
    ...
    reactor = temperature * neutrons_produced_per_second/threshold * 100

    for limit, message in STATUS.items():
        if reactor < limit:
            return message
